    ...
    for line in irf:
        ...
        if ir[qid] != []:
            if ir[qid][-1] != 'NIL':  # Don't add any more results if we've already added the NIL value.
                ir[qid].append(parts[
                                   0])  # that's the candidate id, or NIL. All candidates before the NIL are regarded
            #  as relevant.
        else:
            ir[qid].append(parts[0])
        if parts[-1] == 'true':
            q_rel[qid].append(parts[
                                  0])  # collect the candidate ids of ALL true duplicates, even the ones after the
        # NIL, to be used in truncated MAP.
...
def eval_reranker(res_fname="svm.test.res", pred_fname="svm.train.pred",
    ...
    for i, (p_se, p_svm, a_se, a_svm, a_se1, a_svm1, a_se2, a_svm2) in enumerate(
            zip(prec_se, prec_svm, acc_se, acc_svm, acc_se1, acc_svm1, acc_se2, acc_svm2), 1):
        print( "REC-1@%02d: %6.2f %6.2f  ACC@%02d: %6.2f %6.2f  AC1@%02d: %6.2f %6.2f  AC2@%02d: %4.0f %4.0f" % (
            i, p_se, p_svm, i, a_se, a_svm, i, a_se1, a_svm1, i, a_se2, a_svm2))
    print()
    print( "REC-1 - percentage of questions with at least 1 correct answer in the top @X positions (useful for tasks " \
          "where questions have at most one correct answer)")
    print( "ACC   - accuracy, i.e., number of correct answers retrieved at rank @X normalized by the rank and the " \
          "total number of questions")
    print( "AC1   - the number of correct answers at @X normalized by the number of maximum possible answers (perfect " \
          "re-ranker)")
...
#        print "%16s %6s %6s  %14s %6s %6s  %14s %6s %6s  %12s %4s %4s" % ("IR", "ZERO", "SYS", "IR", "ZERO", "SYS",
# "IR", "ZERO", "SYS", "IR", "ZERO", "SYS")
#        for i, (p_se, p_svm, a_se, a_svm, a_se1, a_svm1, a_se2, a_svm2) in enumerate(zip(prec_se, prec_svm, acc_se,
# acc_svm, acc_se1, acc_svm1, acc_se2, acc_svm2), 1):
#                print "REC-1@%02d: %6.2f %6.2f  ACC@%02d: %6.2f %6.2f  AC1@%02d: %6.2f %6.2f  AC2@%02d: %4.0f %4.0f"
# %(i, p_se, p_svm, i, a_se, a_svm, i, a_se1, a_svm1, i, a_se2, a_svm2)
#        print
#        print "REC-1 - percentage of questions with at least 1 correct answer in the top @X positions (useful for
# tasks where questions have at most one correct answer)"
#        print "ACC   - accuracy, i.e., number of correct answers retrieved at rank @X normalized by the rank and the
#  total number of questions"
#        print "AC1   - the number of correct answers at @X normalized by the number of maximum possible answers (
# perfect re-ranker)"
#        print "AC2   - the absolute number of correct answers at @X"
...
def eval_search_engine(res_fname, format, th=10):
    ...
    print()
    print( "REC-1 - percentage of questions with at least 1 correct answer in the top @X positions (useful for tasks " \
          "were questions have at most one correct answer)")
    print( "ACC   - accuracy, i.e. number of correct answers retrieved at rank @X normalized by the rank and the total " \
          "" \
          "number of questions")
    print( "AC1   - the number of correct answers at @X normalized by the number of maximum possible answers (perfect " \
          "re-ranker)")
